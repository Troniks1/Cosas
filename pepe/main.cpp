...
int main(void){
    ...
    std::ifstream archivo_in{ "pepe.txt" };//archivo de entrada
  ...
  while (std::getline(archivo_in, line)) {
    ...
    if(contador != 0){
        ...
        if((pos = line.find(reempN) != string::npos)){
                line.replace(pos, lenN, reempN_con);//si la encuentra la reemplaza con México
        }
        else if((pos = line.find(reempP) != string::npos)){
            line.replace(pos, lenP, reempP_con);//si la encuentra la reemplaza con México
        }
    }
      tweet += line + " ";
      contador = 1;
